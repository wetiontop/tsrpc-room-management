...
import { Camera, Color, Component, EditBox, instantiate, Label, Node, Prefab, Quat, RichText, tween, TweenSystem, Vec2, _decorator } from 'cc';
...
export class RoomScene extends Component {
    ...
    private _initClient() {
        ...
        this.client.listenMsg('serverMsg/Chat', v => {
            let playerName = this.playerNames.getChildByName(v.user.id)?.getComponent(PlayerName);
            if (playerName) {
